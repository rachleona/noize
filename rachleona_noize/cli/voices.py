...
def add(
    ...
    try:
        ...
        warn(
            "A voice with this id already exists, please try again with a different name"
        )
        raise typer.Exit(1)
    ...
    with_spinner(
        "Calculating audio embeddings", calculate_embs, id, config_file, device
    )
...
def reset(
    ...
    try:
        ...
        warn(
            "No voice exists under this id, please use the [bold]add[/] command to add a new voice instead"
        )
...
def calculate_embs(id, config_file, device):
    ...
    # embeddings
    embs = {}
    embs["ov_embed"] = ov_extract_se(
        openvoice,
        audio,
        data_params.filter_length,
        data_params.hop_length,
        data_params.win_length,
        {},
        device,
    )
    embs["xtts_embed"] = xtts_get_emb(xtts, audio, data_params.filter_length)
    embs["avc_embed"] = avc.get_speaker_embeddings(
        audio, avc_hp, data_params.sampling_rate, device
    )
