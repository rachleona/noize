...
def worker():
    ...
    while worker_active:
        if task_queue:
            ...
            src_srs, _ = librosa.load(job.filepath, sr=perturber.data_params.sampling_rate)
            src_segments = split_audio(src_srs, perturber.DEVICE, perturber.data_params.sampling_rate)
            job.src_srs = src_srs
            ...
            in_progress = job
            p = job.perturber.generate_perturbations(job.src_segments, len(job.src_srs), job.progress_tracker)
            target_audio_srs = job.src_srs + p
            output_filename = re.search(
                "[\\w-]+?(?=\\.)", os.path.basename(job.filepath)
            ).group(0)
            output_filename = os.path.join(outputs_path, f"protected_{ output_filename }.wav")

            sf.write(output_filename, target_audio_srs, job.perturber.data_params.sampling_rate)
...
class ProtectJob:
    def __init__(self, audio_file, perturbation_level, target, avc, freevc, xtts, iterations):
        self.id = uuid.uuid4()
        ...
        if self.avc: self.encoders.append('AdaptVC')
        if self.freevc: self.encoders.append('FreeVC')
        if self.xtts: self.encoders.append('XTTS')
        ...
        self.filepath = os.path.join(src_path, secure_filename(audio_file.filename))
    ...
    def get_metadata(self):
        return {
            ...
            'iterations': self.iterations,
            'target': self.target,
            'src_file': os.path.basename(self.filepath),
